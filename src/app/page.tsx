...
import { 
  ...
  Lightbulb, MapPin, ShieldCheck, LogOut, Settings, User 
} from 'lucide-react';
...
export default function FullyResponsiveDashboard() {
  ...
  // 🛡️ الجدار الناري الفولاذي (طرد فوري وقاطع لأي شخص غير الإدارة) 🛡️
  useEffect(() => {
    ...
    const role = parsedSession.role || 'Employee';
    const allowedAdmins = ['Admin', 'AsstManager', 'Accountant'];
    
    // إذا المستخدم مو من الإدارة العليا، يتم طرده فوراً قبل تحميل أي شيء
    if (!allowedAdmins.includes(role)) {
      if (role === 'BranchManager') {
        window.location.replace('/branch-portal');
      } else if (role === 'Chef') {
        window.location.replace('/kds');
      } else {
        window.location.replace('/my-profile');
      }
      return; // إيقاف تنفيذ الكود نهائياً
    }
    
    // إذا وصل لهنا، معناه هو إدارة عليا
  ...
  const MENU_GROUPS = useMemo(() => [
    ...
    },
    {
      ...
      items: [
        ...
        { href: '/attendance', icon: <UserCheck className="w-6 h-6" />, title: 'سجل الحضور والغياب', desc: 'لوحة الترحيل السريع لتسجيل الحضور، الغياب، والإجازات.', color: 'bg-rose-100 text-rose-600 dark:bg-rose-500/20 dark:text-rose-400 border border-rose-200 dark:border-rose-500/30', roles: ['Admin', 'AsstManager', 'Accountant'] },
    ...
    {
      id: 'system',
      title: "إعدادات النظام والحساب (System & Account)",
      items: [
        { href: '/my-profile', icon: <User className="w-6 h-6" />, title: 'حسابي الشخصي (My Profile)', desc: 'بوابتك الخاصة لعرض راتبك، تفاصيلك، وتقديم طلباتك للإدارة.', color: 'bg-emerald-100 text-emerald-600 dark:bg-emerald-500/20 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30', roles: ['Admin', 'AsstManager', 'Accountant'] },
