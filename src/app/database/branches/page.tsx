...
// 💡 الخوارزمية الجديدة: توليد لون ثابت ومستحيل يتكرر لكل فرع بناءً على الـ ID مالته 💡
function getBranchColor(id: string) {
  ...
  const s = 70 + (Math.abs(hash) % 25); // التشبع بين 70 و 95
  const l = 55 + (Math.abs(hash) % 10); // الإضاءة بين 55 و 65 للـ Dark Mode
...
export default function BranchesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // إذا لم يكن هناك عمود لهذه الحقول الجغرافية في قاعدة البيانات حالياً، ستتجاهلها Supabase أو تظهر خطأ
      // يفضل إضافة هذه الأعمدة لجدول branches لاحقاً لضمان حفظها
      if (isEditing && editId) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#050505] text-slate-900 dark:text-white font-sans relative overflow-x-hidden pb-40 transition-colors duration-300" dir="rtl">
      
      <div className="fixed top-[-10%] left-[-10%] w-[120%] h-[120%] bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-100/50 dark:from-indigo-900/15 via-transparent dark:via-[#050505] to-transparent dark:to-[#050505] -z-10 pointer-events-none transition-colors duration-300"></div>

      <div className="p-4 md:p-8 max-w-[100rem] mx-auto w-full relative z-10">
        
        {/* الهيدر */}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8 bg-white/80 dark:bg-white/5 backdrop-blur-2xl p-6 md:px-8 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-sm dark:shadow-[0_8px_30px_rgba(0,0,0,0.5)] transition-colors duration-300">
        ...
        {!dbError && isLoading ? (
          ...
          <div className="w-full relative z-10">
            ...
            {/* تبويبات الفلترة حسب الوكالة (التبويبات تاخذ لون الوكالة) */}
            ...
            {filteredBranches.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {filteredBranches.map((branch) => {
                  // 💡 هنا السحر: اللون يتولد تلقائياً وبشكل دائم بناءً على ID الفرع 💡
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#121214] border border-slate-200 dark:border-white/10 p-5 md:p-6 rounded-[2rem] w-full max-w-[420px] shadow-2xl dark:shadow-[0_0_50px_rgba(79,70,229,0.15)] animate-in zoom-in-95 duration-300 flex flex-col overflow-hidden transition-colors duration-300">
            
            <div className="flex justify-between items-center mb-4 border-b border-slate-200 dark:border-white/5 pb-4 shrink-0">
              ...
              <button onClick={closeModal} className="p-2 bg-slate-50 dark:bg-white/5 hover:bg-rose-50 hover:text-rose-600 dark:hover:bg-rose-500/20 text-slate-500 dark:text-slate-400 dark:hover:text-rose-400 rounded-xl transition-colors outline-none cursor-pointer active:scale-95">
                <X className="w-4 h-4"/>
              </button>
            </div>
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-1">
              ...
              <div className="border-t border-slate-200 dark:border-white/5 pt-3 mt-1">
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-slate-200 dark:border-white/5 shrink-0 mt-2">
                <button 
                  type="submit" 
                  disabled={isSaving} 
